    ...
    <div class="flex flex-col justify-center items-center">
        <div class="w-full max-w-2xl">
            <x-form wire:submit='save' class="mb-2">
                ...
                <trix-editor input="id22" class="trix-content">{!!  $article->description !!}</trix-editor>
                @error('form.description')
                    <span class="text-danger">{{$message}}</span>
                @enderror
                <x-file wire:model="form.path_image" label="{{__('Cover Image')}}" hint="Only mp4" />
                <x-button label="{{__('Update')}}" class="btn-primary my-2" type="submit" spinner="save" />
            </x-form>
        </div>
</div>
